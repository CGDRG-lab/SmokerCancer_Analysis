...
allowWGCNAThreads()
setwd("~/Reuben/Biomarker Finding/for_cgdrg/wgcna")
...
## ------------------------------ JUST CHECKING SOME GENES LOCATION
i_genes <- c(
	'ADH7', 'NQO1', 'CYP1A1', 'CYP1B1', 'AKR1B10', 'ALDH1A1', 'ALDH3A1', 'NFE2L2', 
	'NRF1', 'MAFG', 'TP53', 'GCLC', 'GCLM', 'ECH1', 'ETFB', 'NDUFV1', 'RBM10'
)
for (g in i_genes){
	print(paste(
		g, '->',
...
length( colnames(edata)[module_col=="darkturquoise"] )

head(colnames(edata)[module_col=="darkturquoise"], n=50)
# write_clip(colnames(edata)[module_col=="darkturquoise"])
...
## ==================== compare with stourfer meta analysis
sig_genes_meta <- readLines('../dge/sig_genes_meta.txt')
...
filtered_gene <- ( abs(GS1) > 0.30 ) & 
	( abs(KMEs[,  paste("MM", which_module, sep='.')]) > 0.80 )
table( filtered_gene )
...
hub_gene_adjacency <- ADJ1[top_hub_gene, top_hub_gene] * 1

hub_gene_adjacency[hub_gene_adjacency < 0.20] <- 0
...
plot(
	...
	vertex.frame.color = adjustcolor("black", alpha.f = 0.20),
	edge.color = adjustcolor("black", alpha.f = 0.30),
